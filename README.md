...
- **Tighter IAM for CI** — the GitHub Actions deploy role currently uses `AdministratorAccess` to get the pipeline working quickly. Narrow it to the specific CloudFormation/Lambda/API Gateway/DynamoDB/IAM/S3 actions `cdk deploy` actually needs.
- **Duplicate `locationId` handling on create** — `POST /locations` upserts via `PutItem`, so creating with an id that already exists silently overwrites it. `create` should check for an existing item first and return `409 Conflict`.
- **Geocoding reliability** — Nominatim's public endpoint has no SLA and enforces strict rate limits (1 request/second, no bulk queries). A production deployment should move to a paid geocoding provider (or a self-hosted Nominatim instance) and cache results.
